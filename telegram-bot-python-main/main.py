...
@bot.message_handler(commands=["confirmar_pagamento"])
def confirmar_pagamento(message):
    if str(message.chat.id) == ADMIN_ID:
        try:
            msg_parts = message.text.split(" ", 1)  # Divide o comando em partes
            target_user_id = msg_parts[1]  # ID do usuário que fez o pagamento
            
            bot.send_message(target_user_id, f"🎉 Parabéns! Seu pagamento foi confirmado. Aqui está seu acesso ao VIP: {VIP_GROUP_LINK}")
            bot.send_message(ADMIN_ID, f"✅ Acesso VIP liberado para {target_user_id}.")
        except:
            bot.send_message(ADMIN_ID, "⚠ Erro: Use o formato correto: `/confirmar_pagamento <user_id>`")
